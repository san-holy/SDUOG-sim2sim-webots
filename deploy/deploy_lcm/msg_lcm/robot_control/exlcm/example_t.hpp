...
int example_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->buffer[0], 15);
    if(tlen < 0) return tlen; else pos += tlen;
...
int example_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->buffer[0], 15);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    int enc_size = 0;
    enc_size += __int32_t_encoded_array_size(NULL, 15);
    return enc_size;
}

uint64_t example_t::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0xb83e36101368ef36LL;
